...
describe('procedural music', () => {
  ...
  it('adds a mid-note swell for sustained synth-pad voices', () => {
    const createdGains: Array<{
      gain: {
        setValueAtTime: ReturnType<typeof vi.fn>;
        exponentialRampToValueAtTime: ReturnType<typeof vi.fn>;
    ...
    try {
      ...
      expect(gainRamps[0]?.[0]).toBeCloseTo(0.055, 4);
      expect(gainRamps[1]?.[0]).toBeCloseTo(0.046, 4);
      expect(gainRamps[2]?.[0]).toBeGreaterThan(gainRamps[1]?.[0] ?? 0);
      expect(gainRamps[3]?.[0]).toBeCloseTo(0.046, 4);
    } finally {
      if (originalAudioContext) {
        vi.stubGlobal('AudioContext', originalAudioContext);
      } else {
  ...
  it('skips sustained swells for short string notes', () => {
    const createdGains: Array<{
      gain: {
        setValueAtTime: ReturnType<typeof vi.fn>;
        exponentialRampToValueAtTime: ReturnType<typeof vi.fn>;
