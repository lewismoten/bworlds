...
- [ ] Reduce Object3D count and unnecessary scene hierarchy depth.
      ...
      fragments into one `InstancedMesh`, its repeated column ring into one
      `InstancedMesh`, and taller column caps into one `InstancedMesh`, tall
      ships now collapse their repeated masts, yards, and sails into instanced
      sets, and `tile-lighthouse` now collapses its repeated lantern-room
      frame posts, balcony rail posts, four lantern-room panes, and four
      wall-glow boxes into instanced sets, and full-detail forest stone-ring
      landmarks now collapse their repeated stones into one `InstancedMesh`,
      and full-detail forest mushroom-ring landmarks now collapse their
      repeated stems and caps into two `InstancedMesh` nodes, which removes
