...
export function createQuarryTilePlugin(): RuntimePlugin {
  return createAnchoredEnterablePoiTilePlugin({
    ...
    create3DModel(context: Create3DModelContext) {
      return runQuarryModelBuildToCompletion(
        createQuarryModelProgressive(context)
      );
    },
    create3DModelProgressive(context: Create3DModelContext) {
      return createQuarryModelProgressive(context);
    },
...
function* createQuarryModelProgressive({
  three,
  state,
  tileX,
  tileY,
